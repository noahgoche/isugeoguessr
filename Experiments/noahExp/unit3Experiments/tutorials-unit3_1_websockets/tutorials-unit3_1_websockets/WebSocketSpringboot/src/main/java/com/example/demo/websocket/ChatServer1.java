...
public class ChatServer1 {
    ...
    public void onOpen(Session session, @PathParam("username") String username) throws IOException {
        ...
        if (usernameSessionMap.containsKey(username)) {
            ...
            logger.info("[onOpen] " + username + " is already logged in.");
            session.close();
        }
        else {
    ...
    public void onMessage(Session session, String message) throws IOException {
        ...
        // Turns message into a priority for everyone "!! <message>"
        else if (message.startsWith("!!")) {
            broadcast("PRIORITY MESSAGE !" + message.substring(2).trim().toUpperCase() + "!");
        }
        else if(message.equalsIgnoreCase("/exit"))
        {
            sendMessageToPArticularUser(username, "Disconnected");
            onClose(session);
        }
